...
import math   # 추가 05.21_pjs
from absl import logging    # 추가 06.10_pjs
import functools            # 06.10_pjs
from tensorflow.python.distribute import distribute_lib     # 06.10_pjs
from object_detection import model_lib
from object_detection import model_lib_v2

import shutil # 06.26_pjs
import re   # 06.30_pjs
import logging		# pjs

MODEL_BUILD_UTIL_MAP = model_lib.MODEL_BUILD_UTIL_MAP
NUM_STEPS_PER_ITERATION = 200   # JH: 기본 100
patience = 100
...
def eager_train_step(detection_model,
                     ...
                     num_replicas=1.0,
                     summary_writer=None,   # 06.10_pjs
                     global_step=None):     # 06.10_pjs
...
def train_loop(
    ...
    model_dir,
    fine_tune_checkpoint=None,  # 06.26_pjs (재개우선 or 신규시작)
  ...
  print(f"[TOTAL_STEPS] {train_steps}", flush=True)  # 추가_ 콘솔 출력용_pjs  

  # (1) annos 폴더 경로 설정 _06.20_pjs
  ...
  # 06.30 _1: 파일 열기부터 파싱까지 한번에 처리_pjs
  try:
  ...
  with strategy.scope():
    ...
    def train_dataset_fn(input_context):
      ...
      #train_input = train_input.prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
  ...
  with summary_writer.as_default():
    with strategy.scope():
      with tf.compat.v2.summary.record_if(
        ...
        # in a worker. 
        latest_checkpoint = tf.train.latest_checkpoint(manager_dir)	# _pjs (ckpt 깔끔하게 보관 및 관리)
        ...
        def train_step_fn(features, labels):
          ...
          losses_dict = eager_train_step(
              ...
              summary_writer=summary_writer,      # 06.10_pjs
              global_step=global_step)      # 06.10_pjs
        ...
        gs = int(global_step.value()) # _pjs
        logged_step = global_step.value()

        total_loss_list = [0]
        wait = 0				# 학습 중 best_loss 가 갱신 되지 않았을 때 1씩 증가
        i = 0
        best_loss = 1
        total_time = 0
        str_tf_version = tf.__version__


        last_step_time = time.time()
        # 추가_ProgressBar를 위한 총 스텝 출력 _pjs
        print(f"[TOTAL_STEPS] {train_steps}", flush=True)
        
        for _ in range(global_step.value(), train_steps, num_steps_per_iteration): 
          losses_dict = _dist_train_step(train_input_iter)
          # 기존 Step 출력 _추가 05.09 _pjs
          print(f"[Step] {int(global_step.value())}", flush=True)   

          # 현재 Loss 출력 (ProgressBar) _추가 05.12 _pjs
          current_loss = float(losses_dict['Loss/total_loss'])
          print(f"[LOSS] {current_loss:.7f}", flush=True)      

          # 06.30 _pjs
          gs = int(global_step.value())
          
          # 06.11 추가 _pjs
          # Loss/total_loss 요약 기록 시작 _pjs
          with summary_writer.as_default():
            tf.compat.v2.summary.scalar(
                'Loss/total_loss', current_loss, step=int(global_step.value())      # 06.12 step=global 이였는데 저거 추가함
            )
            summary_writer.flush()

          # Nan 값 추가_25.05.21 _pjs
          if math.isnan(current_loss):
            print(f"[NaN DETECTED] Step {global_step}에서 loss 값이 NaN입니다. 훈련을 중단합니다.", flush=True)
            sys.exit()
          ...
          # steps_per_sec_list.append(steps_per_sec)    # 2022_0817 JH
          ...
          # if global_step.value() - logged_step == num_steps_per_iteration:    # 07.02 (origin)
          if global_step.value() - logged_step >= num_steps_per_iteration:
		  	# 로그 출력, tf.summary 기록, 주기적 체크포인트 저장 등 실행_pjs
            logged_dict_np = {name: value.numpy() for name, value in
                              logged_dict.items()}
            total_time += time_taken
            average_time = float(total_time) / ((int(global_step.value()) / 200))
            '''tf.logging.info(
                 str_tf_version + 'wait {} Step {} steps time {:.3f}s total-time{:.4f}h'.format(
                    wait, global_step.value(), time_taken, total_time/3600))'''    # JH: 1 step 당 시간 : time_taken / num_steps_per_iteration

            tf.logging.info(f"tensorflow Version:{str_tf_version} |  Wait:{wait} | Step:{global_step.value()} | step_time:{time_taken:.3f}s | avg_time:{average_time:.4f}s | total_time:{total_time/3600:.5f}h")
            tf.logging.info(pprint.pformat(logged_dict_np, width=40))
            logged_step = global_step.value()

            i += 1
            total_loss_list.append(float(losses_dict['Loss/total_loss']))

            # train_steps 는 앞에서 동적으로 계산된 최종 스텝 수 (예: num_images*20, 상·하한 적용 완료) _06.23_pjs
            early_start = int(train_steps * 0.7)  # 전체의 70% 지점에서부터 조기 종료 시작_pjs
            # 2023.02.20 양부장님,mk 코드 갱신
            # 훈련시 반드시 조절할것  _pjs       
            if global_step >= early_start:#훈련시 반드시 조절할것 -> 이곳은 최적 손실율을 탐지하기 시작하는 루틴, 즉 이 스텝이후 훈련이 종료됨
              if total_loss_list[i] < best_loss:
                  best_loss = total_loss_list[i]
                  
            if best_loss<0.1 :
                #일정 스텝이상 일때, 그리고 Total loss값이 일정 수준 이하일때
                if (total_loss_list[i] >= total_loss_list[i - 1]) or (total_loss_list[i] >= best_loss):
                    wait += 1                    

                elif total_loss_list[i] < best_loss:
                    best_loss = total_loss_list[i]
                    wait = 0

            if (wait >= patience):
            #2023.01.04 부장님 학습 트레인 끝나고 터미널에 학습 끝낫다고 표기할수 있도록 지시
                manager.save(int(global_step.value() // checkpoint_every_n))  # 마지막 강제 저장 | 조기종료도 저장
                print("It's done. Do NOT need to update Best Loss", flush=True)
                raise SystemExit  # os._exit(0)은 socket 에러, 대신 안전한 종료로 교체
                              
          #YSJ: Need to Test : Save Only when 'the total_loss_list[i]' value is less than 0.1 AND Should be less than 'best_loss'
          # 마지막 체크포인트 저장 루틴_05.29 (global_step값 이상이면 무조건 저장)_pjs
            # if global_step >= early_start:#훈련시 반드시 조절할것 -> 이곳은 손실율 0.1 이하일때 저장하는 루틴  
            if global_step >= early_start:
              if gs >= early_start and gs % checkpoint_every_n == 0:
                ...
                print(f"[CHECKPOINT SAVED → ckpt-{cp_idx} (step {gs})]", flush=True)

    # ── train_loop 전체 반복문 & early-stop 분기 끝 ──_pjs
    # 정상적으로 모든 step을 돌았을 때도 SIGNAL을 찍고 manager를 반환_pjs
