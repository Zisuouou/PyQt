...
import math   # 추가 05.21
from absl import logging    # 추가 06.10
import functools            # 06.10
from tensorflow.python.distribute import distribute_lib     # 06.10
from object_detection import model_lib
...
def eager_train_step(detection_model,
                     ...
                     num_replicas=1.0,
                     summary_writer=None,   # 06.10
                     global_step=None):     # 06.10
  ...
  # 06.10 _TensorBoard Loss 가져오는거 추가
...
def train_loop(
    ...
    model_dir,
    fine_tune_checkpoint=None,  # 06.26
  ...
  print(f"[TOTAL_STEPS] {train_steps}", flush=True)  # 추가_ 콘솔 출력용  

  # (1) annos 폴더 경로 설정 _06.20
  ...
  with summary_writer.as_default():
    with strategy.scope():
      with tf.compat.v2.summary.record_if(
        ...
        # in a worker.
        latest_checkpoint = tf.train.latest_checkpoint(manager_dir)
        ...
        def train_step_fn(features, labels):
          ...
          losses_dict = eager_train_step(
              ...
              summary_writer=summary_writer,      # 06.10
              global_step=global_step)      # 06.10
        ...
        for _ in range(global_step.value(), train_steps, num_steps_per_iteration): 
          ...
          # 기존 Step 출력 _추가 05.09
          print(f"[Step] {int(global_step.value())}", flush=True)

          # 현재 Loss 출력 (ProgressBar) _추가 05.12
          ...
          if global_step.value() - logged_step >= num_steps_per_iteration:
            ...
            # train_steps 는 앞에서 동적으로 계산된 최종 스텝 수 (예: num_images*20, 상·하한 적용 완료) _06.23
            early_start = int(train_steps * 0.7)  # 전체의 70% 지점에서부터 조기 종료 시작
          ...
          # 마지막 체크포인트 저장 루틴_05.29 (global_step값 이상이면 무조건 저장)
    ...
    # ── train_loop 전체 반복문 & early-stop 분기 끝 ──
    # 정상적으로 모든 step을 돌았을 때도 SIGNAL을 찍고 manager를 반환
